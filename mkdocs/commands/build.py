...
def build(config, live_server=False, dirty=False):
    ...
    if utils.error_filter.count:
        raise SystemExit('\nExited with {} errors and {} warnings.'.format(
            utils.error_filter.count, utils.warning_filter.count)
        )
    elif utils.warning_filter.count and config['strict']:
